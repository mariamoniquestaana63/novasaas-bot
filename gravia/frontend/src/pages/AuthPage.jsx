...
export default function AuthPage() {
  ...
  return (
    <div className="min-h-screen bg-surface flex flex-col items-center justify-center px-4 font-mono">
      {/* Back */}
      <Link to="/" className="text-gray-600 text-xs hover:text-gray-400 transition mb-8 self-start ml-auto mr-auto" style={{ maxWidth: 400, width: "100%" }}>
        ← gravia
      </Link>

      <div className="w-full max-w-sm">
        <div className="bg-surface2 border border-white/8 rounded-xl overflow-hidden">
          {/* Tabs */}
            ...
            {["login", "signup"].map((t) => (
              <button
                ...
                className={`flex-1 py-3.5 text-xs font-bold uppercase tracking-widest transition ${
                  tab === t ? "text-brand border-b-2 border-brand" : "text-gray-500 hover:text-gray-300"
                }`}
              >
            ...
            {done ? (
              <div className="text-center py-4">
                ...
                <p className="text-gray-400 text-xs leading-relaxed">
                  We've sent a confirmation link to <span className="text-brand">{email}</span>. Click it to activate your account.
                </p>
              </div>
            ) : (
              <form onSubmit={submit} className="flex flex-col gap-4">
                {tab === "signup" && (
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-surface3 border border-white/8 rounded-lg px-4 py-2.5 text-sm text-white placeholder-gray-600 outline-none focus:border-brand/40 transition"
                    />
                  </div>
                )}
                <div>
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="bg-brand text-black font-bold py-2.5 rounded-lg text-sm hover:opacity-90 transition disabled:opacity-60 mt-1"
                >
                ...
                {tab === "signup" && (
                  <p className="text-gray-600 text-[11px] text-center leading-relaxed">
                    By signing up you agree to our Terms of Service. 14-day free trial on Pro.
                  </p>
                )}
              </form>
            )}
