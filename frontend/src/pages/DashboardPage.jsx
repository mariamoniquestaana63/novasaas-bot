          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="flex flex-col gap-3">
              <div className="bg-surface2 border border-white/5 rounded-lg p-4">
                ...
                {!isPro && (
                  <button onClick={() => navigate("/pricing")} className="mt-3 w-full bg-[#00ff88] text-black font-bold text-[10px] py-2 rounded hover:opacity-90 transition">Upgrade →</button>
                )}
                {isPro && (
                  <button onClick={openPortal} className="mt-3 w-full border border-white/10 text-gray-400 text-[10px] py-2 rounded hover:border-white/20 transition">Manage billing</button>
                )}
              </div>
              <div className="bg-surface2 border border-white/5 rounded-lg p-4">
            ...
            <div className="bg-surface2 border border-white/5 rounded-lg p-5 max-w-2xl">
              ...
              <div className="space-y-4 text-[11px]">
                {[
                  ...
                  { method: "POST", path: "/api/checkout",        desc: "Create Stripe Checkout session" },
                  { method: "POST", path: "/api/portal",          desc: "Open Stripe Customer Portal" },
                  { method: "POST", path: "/api/stripe/webhook",  desc: "Stripe webhook receiver" },
                ].map(e => (
                  <div key={e.path} className="flex gap-3 items-start">
                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded flex-shrink-0 ${e.method === "GET" ? "bg-[#00ff88]/10 text-[#00ff88]" : "bg-brand2/20 text-brand2"}`}>{e.method}</span>
                    <div>
